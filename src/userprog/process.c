...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  if(child_tid < 0 || child_tid > 999999) return -1;
  for(t = list_begin(&(thread_current()->child)); t!=list_end(&(thread_current()->child)); t=list_next(t)){
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  strlcpy(command, file_name, sizeof(command));
  for (argv[cnt] = strtok_r(command, " ", &save); argv[cnt] != NULL; argv[++cnt] = strtok_r(NULL, " ", &save));
  
  file_exec = argv[0];
  /* Open executable file. */
  file = filesys_open (file_exec);
  if (file == NULL) 
    {
  ...
  if(total_len % 4 != 0){
    int word_align = 4 - ((total_len) % 4);
    for(int j=0;j<word_align;j++)
    {
      *esp -= 1;
      **(uint8_t **)esp = 0;
    }
  ...
  **(uint32_t**)esp = 0;
  ...
  for (int i = cnt - 1; i >= 0; i--) {
      ...
      esp_temp -= len;
      *esp -= 4;
      **(uint32_t**)esp = (uint32_t)esp_temp;
  }
